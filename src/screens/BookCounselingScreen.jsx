...
export default function BookCounselorScreen({
  ...
  const loadCounsellors = async () => {
    try {
      ...
      Alert.alert(translate("Error"), e?.message || "Could not load counsellors");
    } finally {
      setLoading(false);
    }
  };
  useEffect(() => {
    if (!requestId) {
      Alert.alert(translate("Missing requestId"), translate("Counseling requestId was not received. Please submit the counseling form again."), [{
        text: translate("OK"),
  ...
  const handleSubmit = async () => {
    try {
      if (!requestId) {
        Alert.alert(translate("Missing request"), translate("No requestId found. Please submit the counseling form again."));
        return;
      }
      const counsellorId = getId(selectedCounsellor);
      if (!counsellorId) {
        Alert.alert(translate("Pick counsellor"), translate("Please select a counsellor first."));
        return;
      }
      ...
      Alert.alert(translate("Session Booked ✅"), `${translate("Counsellor")}: ${selectedCounsellor?.fullName || translate("Counsellor")}\n${translate("Request ID")}: ${requestId}\n${translate("Date")}: ${translate(month)} ${selectedDay}, ${selectedYear}\n${translate("Time")}: ${selectedSlot}\n${translate("Status")}: ${translate(data?.appointment?.status || "pending")}`, [{
        text: translate("OK"),
        onPress: () => navigation.goBack()
      }]);
    } catch (e) {
      Alert.alert(translate("Booking failed"), e?.message || "Could not book counselling session");
    } finally {
      setBooking(false);
    }
  };
    ...
    return <SafeAreaView style={[styles.container, {
      backgroundColor: UI.bg
        ...
        justifyContent: "center"
          ...
          color: UI.mut,
          fontWeight: "700"
        }}>{translate("Loading counsellors…")}</Text>
        </View>
      </SafeAreaView>;
  }
  return <SafeAreaView style={[styles.container, {
    backgroundColor: UI.bg
      ...
      <ScrollView style={styles.body} contentContainerStyle={styles.bodyContent} showsVerticalScrollIndicator={false}>
        <Text style={styles.screenTitle}>{translate("Book Session.")}</Text>

        {/* COUNSELLOR LIST */}
        <Text style={styles.label}>{translate("Select Counsellor")}</Text>

        {counsellors.length === 0 ? <View style={[styles.emptyBox, {
        backgroundColor: UI.card
      }]}>
            <Text style={{
          color: UI.mut,
          fontWeight: "700"
        }}>{translate("No counsellors found. Ask admin to create counsellor accounts.")}</Text>
        ...
        paddingBottom: 6
          ...
          return <TouchableOpacity key={id} activeOpacity={0.9} onPress={() => setSelectedCounsellor(c)} style={[styles.counsellorCard, active && {
            borderColor: ORANGE,
            borderWidth: 1.5
          }]}>
                  <View style={styles.avatar} />
                  <Text style={styles.counsellorName}>{c?.fullName || translate("Counsellor")}</Text>
        ...
        <View style={styles.calendarCard}>
          ...
          <View style={styles.monthBar}>
            ...
            {/* Month dropdown */}
            <TouchableOpacity style={styles.monthCenter} onPress={() => setMonthOpen(p => !p)} activeOpacity={0.9}>
              <Text style={styles.monthTitle}>
                {translate(MONTHS[selectedMonthIndex])} {selectedYear}
              </Text>
          ...
          {/* Month dropdown list */}
          {monthOpen && <View style={styles.monthDropdown}>
              <ScrollView style={{
            maxHeight: 220
          }} showsVerticalScrollIndicator={false}>
                {MONTHS.map((m, idx) => {
              const active = idx === selectedMonthIndex;
              return <TouchableOpacity key={m} style={[styles.monthItem, active && styles.monthItemActive]} onPress={() => {
                setSelectedMonthIndex(idx);
                setMonthOpen(false);
                setSelectedDay(null);
                setSelectedSlot("");
              }}>
                  <Text style={[styles.monthItemText, active && styles.monthItemTextActive]}>{translate(m)}</Text>
                    </TouchableOpacity>;
            })}
              </ScrollView>
              ...
              <View style={styles.yearRow}>
                <TouchableOpacity style={styles.yearBtn} onPress={() => {
              setSelectedYear(y => y - 1);
              setSelectedDay(null);
              setSelectedSlot("");
            }}>
                  <Icon name="minus" size={16} color="#111" />
                  <Text style={styles.yearBtnText}>{translate("Year")}</Text>
                </TouchableOpacity>
          ...
          <View style={styles.daysGrid}>
            ...
            return <TouchableOpacity key={`day-${cell}`} style={[styles.dayCell, isSelected && styles.dayCellSelected]} onPress={() => setSelectedDay(cell)} activeOpacity={0.85}>
        ...
        marginTop: 24
          ...
          <TouchableOpacity style={styles.dropdown} onPress={() => setSlotOpen(prev => !prev)} activeOpacity={0.9}>
