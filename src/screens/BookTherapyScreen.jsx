...
export default function BookTherapyScreen({ navigation, route }) {
  ...
  useEffect(() => {
    if (!requestId) {
      Alert.alert(
        "Missing requestId",
        "Therapy requestId was not received. Please submit the therapy form again.",
    ...
    return (
      <SafeAreaView style={[styles.container, { backgroundColor: UI.bg }]}>
        <View style={{ flex: 1, alignItems: "center", justifyContent: "center" }}>
          ...
          <Text style={{ marginTop: 10, color: UI.mut, fontWeight: "700" }}>Loading therapists…</Text>
        </View>
      </SafeAreaView>
    );
  }
...
const styles = StyleSheet.create({
  ...
  therapistName: { fontSize: 13, fontWeight: "900", color: "#111" },
  therapistMeta: { marginTop: 4, fontSize: 11, fontWeight: "700", color: "#666" },
