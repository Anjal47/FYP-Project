...
export default function WasteReportScreen({
  ...
  const pickReportCode = resp => resp?.reportCode || resp?.code || resp?.data?.reportCode || resp?.report?.reportCode || resp?.report?._id || resp?._id || null;
  const resetForm = () => {
    setDescription("");
    setLocation("");
    setGeoLocation(null);
  ...
  const onSubmit = async () => {
    try {
      ...
      Alert.alert(translate("Submitted"), `Waste report sent successfully.\n\nReport Code: ${reportCode}`, [{
        ...
        onPress: () => {
          ...
          navigation.navigate("WasteReportStatus", {
            reportCode
          });
        }
      }, {
  ...
  return <SafeAreaView style={styles.container}>
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        <ReportHero title={translate("Waste Report")} subtitle={translate("Waste reporting now uses the same cleaner structure as the rest of the app.")} onBack={() => navigation.goBack?.()} styles={styles} theme={theme} />

        <View style={styles.panel}>
          <Text style={styles.sectionTitle}>{translate("What happened?")}</Text>
          <TextInput style={[styles.input, styles.textarea]} placeholder={translate("Describe the waste issue...")} placeholderTextColor={theme.muted} multiline value={description} onChangeText={setDescription} />
