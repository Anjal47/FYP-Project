...
import { SafeAreaView, View, Text, TouchableOpacity, StyleSheet, ScrollView, Alert, ActivityIndicator, RefreshControl, TextInput } from "react-native";
...
const statusUI = statusRaw => {
  ...
  if (s === "approved" || s === "confirmed") return {
    text: "Approved",
    bg: "#EAF7EE",
    fg: "#197A3A"
  };
  if (s === "rejected" || s === "cancelled" || s === "canceled") return {
    text: "Rejected",
    bg: "#FCECEC",
    fg: "#B42318"
  };
...
export default function UserBookedCounselingScreen({
  navigation
}) {
  ...
  const load = async () => {
    try {
      ...
      Alert.alert(translate("Error"), e?.message || "Could not load booked sessions");
    } finally {
      setLoading(false);
    }
  };
  ...
  const onSubmitReview = async appointmentId => {
    ...
    try {
      ...
      setExpandedReviewId("");
      Alert.alert(translate("Review saved"), translate("Your counsellor review has been submitted."));
    } catch (e) {
      Alert.alert(translate("Review failed"), e?.message || "Could not submit your review");
    } finally {
    ...
    backgroundColor: UI.bg
      ...
      <View style={styles.header}>
        <TouchableOpacity style={styles.backRow} onPress={() => navigation.goBack()}>
          ...
          <Text style={styles.headerTitle}>
            <Text style={styles.headerHighlight}>{translate("Counseling")}</Text>
          </Text>
        </TouchableOpacity>
      </View>

      <ScrollView style={styles.body} contentContainerStyle={styles.bodyContent} showsVerticalScrollIndicator={false} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={onRefresh} />}>
        <Text style={styles.screenTitle}>{translate("Booked Sessions.")}</Text>

        {loading ? <View style={styles.loadingWrap}>
            <ActivityIndicator size="large" color={UI.orange} />
            <Text style={styles.loadingTxt}>{translate("Loading booked sessions…")}</Text>
          </View> : appointments.length === 0 ? <View style={[styles.emptyCard, {
        backgroundColor: UI.card
      }]}>
            <Text style={styles.emptyTitle}>{translate("No booked sessions")}</Text>
            <Text style={styles.emptySub}>{translate("When you book counselling, sessions will show up here.")}</Text>

            <TouchableOpacity activeOpacity={0.9} style={styles.reloadBtn} onPress={load}>
              <Icon name="rotate-ccw" size={18} color="#111" />
              <Text style={styles.reloadTxt}>{translate("Reload")}</Text>
            </TouchableOpacity>
          </View> : <View style={{
        gap: 14
      }}>
          ...
          return <TouchableOpacity key={key} activeOpacity={0.95} style={[styles.sessionCard, {
            backgroundColor: UI.card
          }]} onPress={() => {
            Alert.alert(translate("Session Details"), `Counsellor: ${safe(counsellorName)}\nStatus: ${safe(a?.status)}\nMode: ${safe(modeValue)}\nDate: ${safe(a?.month)} ${safe(a?.day)}\nTime: ${safe(a?.slot)}\nRequestId: ${safe(a?.requestId?._id || a?.requestId || "")}\n\nCreated: ${safe(a?.createdAt || "")}`, [{
              text: translate("OK")
            }]);
          }}>
                  {/* top row */}
                ...
                flex: 1
                    ...
                    {isApproved(a?.status) && isOnlineMode(modeValue) ? <TouchableOpacity activeOpacity={0.85} style={styles.msgBtn} onPress={() => {
                navigation.navigate("CounselingChat", {
                  ...
                  counsellorPhone: String(a?.counsellor?.phone || "")
                });
              }}>
                ...
                backgroundColor: st.bg
                  ...
                  <View style={styles.infoRow}>
                    <View style={styles.infoItem}>
                      <Text style={styles.infoLabel}>{translate("RequestId")}</Text>
                    ...
                    <View style={styles.infoItem}>
                      <Text style={styles.infoLabel}>{translate("Session ID")}</Text>
                      <Text style={styles.infoValue} numberOfLines={1}>
                        {safe(a?.id || a?._id)}
                      </Text>
                    </View>
                  ...
                  {isCompleted(a?.status) ? <View style={styles.reviewBox}>
                      <Text style={styles.reviewHeading}>{translate("Your Review")}</Text>
                      {a?.myReview ? <>
                          ...
                          <TouchableOpacity activeOpacity={0.85} style={styles.reviewToggleBtn} onPress={() => setExpandedReviewId(prev => prev === key ? "" : key)}>
                            <Text style={styles.reviewToggleTxt}>
                              {expandedReviewId === key ? "Hide Review Form" : "Rate Counsellor"}
                            </Text>
                          </TouchableOpacity>

                          {expandedReviewId === key ? <View style={styles.reviewForm}>
                              <View style={styles.starsRow}>
                                {[1, 2, 3, 4, 5].map(star => {
                      const active = star <= Number(reviewDrafts[key]?.rating || 0);
                      return <TouchableOpacity key={`${key}-star-${star}`} onPress={() => updateReviewDraft(key, {
                        rating: star
                      })} activeOpacity={0.8}>
                              ...
                              <TextInput value={reviewDrafts[key]?.comment || ""} onChangeText={text => updateReviewDraft(key, {
                    comment: text
                  })} placeholder={translate("Write a short review...")} placeholderTextColor="#999" multiline style={styles.reviewInput} />

                              <TouchableOpacity activeOpacity={0.9} style={[styles.reviewSubmitBtn, submittingReviewId === key && {
                    opacity: 0.7
                  }]} onPress={() => onSubmitReview(key)} disabled={submittingReviewId === key}>
      ...
      <View style={styles.bottomBar}>
        <TouchableOpacity style={styles.tabItem} onPress={handleSettingsPress} activeOpacity={0.8}>
          ...
          <Text style={styles.tabLabel}>{translate("Settings")}</Text>
        </TouchableOpacity>
        ...
        <TouchableOpacity style={styles.tabItem} onPress={handleHomePress} activeOpacity={0.8}>
          ...
          <Text style={[styles.tabLabel, styles.tabLabelActive]}>{translate("Home")}</Text>
        </TouchableOpacity>
