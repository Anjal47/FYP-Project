...
function isTrafficReport(report) {
  const text = `${report?.type || ""} ${report?.category || ""} ${report?.module || ""} ${report?.tag || ""}`.toLowerCase();
  return text.includes("traffic") || text.includes("road") || text.includes("violation") || text.includes("driving");
}
export default function TrafficReportStatusScreen({
  navigation
}) {
  ...
  const showStatusAlert = report => {
    Alert.alert(translate("Traffic Report Status"), `Report: ${report?.reportCode || report?.id}\nType: ${report?.type}\nArea: ${report?.area}\nPriority: ${report?.priority}\n\nStatus: ${report?.status}\nUpdated: ${report?.time || "recently"}`);
  };
  ...
  const trafficReports = useMemo(() => myReports.filter(isTrafficReport), [myReports]);
  const filteredReports = useMemo(() => {
    const q = search.trim().toLowerCase();
    if (!q) return trafficReports;
    return trafficReports.filter(report => [report?.reportCode, report?.id, report?.type, report?.area, report?.status, report?.priority, report?.description].filter(Boolean).join(" ").toLowerCase().includes(q));
  }, [trafficReports, search]);
  const handleCheckStatus = async () => {
    ...
    try {
      ...
      if (live && !isTrafficReport(live)) {
        Alert.alert(translate("Not a Traffic Report"), translate("This report ID does not belong to the traffic module."));
        return;
      }
  ...
  return <SafeAreaView style={styles.container}>
      ...
      loadMyReports(false);
        ...
        <View style={styles.hero}>
          ...
          <Text style={styles.eyebrow}>{translate("Traffic Status")}</Text>
          <Text style={styles.title}>{translate("Track road and traffic reports with less noise and clearer scanning.")}</Text>
          <Text style={styles.subtitle}>{translate("Enter a report code or browse your traffic-specific reports below.")}</Text>
        </View>

        <View style={styles.panel}>
          <Text style={styles.sectionTitle}>{translate("Check a traffic report")}</Text>
          <View style={styles.inputRow}>
            <Icon name="hash" size={16} color={theme.muted} />
            <TextInput style={styles.input} placeholder={translate("e.g. TR-2026-000123")} placeholderTextColor={theme.muted} value={reportId} onChangeText={setReportId} autoCapitalize="characters" />
          </View>
        ...
        <View style={styles.panel}>
          <View style={styles.sectionHeader}>
            <Text style={styles.sectionTitle}>{translate("My traffic reports")}</Text>
          ...
          <View style={styles.inputRow}>
            ...
            <TextInput value={search} onChangeText={setSearch} placeholder={translate("Search by ID, area, status...")} placeholderTextColor={theme.muted} style={styles.input} />
          </View>

          {!listLoading && filteredReports.length === 0 ? <View style={styles.emptyState}>
              <Icon name="file-text" size={18} color={theme.accentStrong} />
              <Text style={styles.emptyText}>{translate("No traffic reports found.")}</Text>
            </View> : null}

          {filteredReports.map(report => {
          const code = report?.reportCode || report?.id || "—";
          return <TouchableOpacity key={report?._id || code} style={styles.reportCard} onPress={() => {
            setReportId(code);
            showStatusAlert(report);
          }} activeOpacity={0.9}>
                <View style={styles.reportCopy}>
                  <Text style={styles.reportCode}>{code}</Text>
                  <Text style={styles.reportMeta}>{report?.type || "Traffic report"} • {report?.area || "Unknown area"}</Text>
